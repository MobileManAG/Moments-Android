...
public class SessionConfig {
    ...
    private boolean fbStoryCreationEnabled;

    public SessionConfig(int hlsSegmentDuration) {
        this.mHlsSegmentDuration = hlsSegmentDuration;
        ...
        outputDir.mkdir();
        mMuxer = FFmpegMuxer.create(outputFile.getAbsolutePath(), Muxer.FORMAT.MPEG4, hlsSegmentDuration);
        mStream = new Stream();
    }
    ...
    public boolean isFbStoryCreationEnabled() {
        return fbStoryCreationEnabled;
    }

    public void setFbStoryCreationEnabled(boolean fbStoryCreationEnabled) {
        this.fbStoryCreationEnabled = fbStoryCreationEnabled;
    }

    public void setmVideoConfig(VideoEncoderConfig videoConfig) {
        this.mVideoConfig = checkNotNull(videoConfig);
    }
    ...
    public static class Builder {
         ...
         */
        public Builder(String outputLocation, int mHlsSegmentDuration) {
            setAVDefaults();
            setMetaDefaults();
            mUUID = UUID.randomUUID();

            if (outputLocation.contains("rtmp://")) {
                mMuxer = FFmpegMuxer.create(outputLocation, Muxer.FORMAT.RTMP, mHlsSegmentDuration);
            } else if (outputLocation.contains(".flv") /*|| outputLocation.contains("f4v") */) {
                mMuxer = FFmpegMuxer.create(createRecordingPath(outputLocation), Muxer.FORMAT.RTMP, mHlsSegmentDuration);
