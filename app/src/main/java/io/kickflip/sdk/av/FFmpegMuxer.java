...
public class FFmpegMuxer extends Muxer implements Runnable {
    ...
    private FFmpegMuxer(String outputFile, FORMAT format, int mHlsSegmentDuration) {
        super(outputFile, format);
        mReady = false;
        mFFmpeg = new FFmpegWrapper();

        FFmpegWrapper.AVOptions opts = new FFmpegWrapper.AVOptions();
        opts.hlsSegmentDurationSec = mHlsSegmentDuration;
        switch (mFormat) {
            case MPEG4:
    ...
    public static FFmpegMuxer create(String outputFile, FORMAT format, int mHlsSegmentDuration) {
        return new FFmpegMuxer(outputFile, format, mHlsSegmentDuration);
    }
