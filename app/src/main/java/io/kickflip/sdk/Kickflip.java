 ...
 * {@link #setup(android.content.Context, String, String, FBUser, io.kickflip.sdk.api.KickflipCallback)}. These tokens are available in your kickflip
 * account dashboard.
 * <h2>Example Usage</h2>
 * <b>Starting a single live broadcast</b>
 * <p/>
 * <ol>
 * <li>{@link #setup(android.content.Context, String, String, FBUser, io.kickflip.sdk.api.KickflipCallback)}</li>
 ...
 * </ol>
 * The {@link com.mobileman.moments.android.frontend.activity.BroadcastActivity} will present a standard camera UI with controls
...
//        return setup(context, key, secret, null);
 //   }
     ...
     */
    public static KickflipApiClient setup(Context context, FBUser fbUser, KickflipCallback cb) {
        sContext = context;
        setApiCredentials("xxx", "yyy");
        return getApiClient(context, fbUser, cb);
    }
    ...
    /**
     * Start {@link com.mobileman.moments.android.frontend.activity.BroadcastActivity}. This Activity
     * facilitates control over a single live broadcast.
     * <p/>
     * <b>Must be called after Kickflip#setup or
     *
     ...
     *                 broadcast events
     */
    public static void startBroadcastActivity(Activity host, BroadcastListener listener, Bundle argumentsBundle) {
        ...
        if (argumentsBundle != null) {
            broadcastIntent.putExtras(argumentsBundle);
        }
    ...
    /**
     * Start {@link com.mobileman.moments.android.frontend.activity.MediaPlayerActivity}. This Activity
     * facilitates playing back a Kickflip broadcast.
     * <p/>
     * <b>Must be called after Kickflip#setup
     *
     * @param host      the host {@link android.app.Activity} initiating this action
     ...
     */
    public static void startMediaPlayerActivity(Activity host, String streamUrl, String thumbnailUrl, String userId, boolean newTask) {
        Intent playbackIntent = new Intent(host, MediaPlayerActivity.class);
        playbackIntent.putExtra(MediaPlayerActivity.MEDIA_URL, streamUrl);
        playbackIntent.putExtra(MediaPlayerActivity.USER_ID, userId);
        playbackIntent.putExtra(MediaPlayerActivity.THUMBNAIL_URL, thumbnailUrl);
        if (newTask) {
            playbackIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NO_HISTORY | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        }
        host.startActivity(playbackIntent);
    }

    public static void startMediaPlayerActivityWithHistoricalStream(Activity host, final HlsStream stream) {
        Intent playbackIntent = new Intent(host, MediaPlayerActivity.class);
        Bundle bundle = new Bundle();
        bundle.putSerializable(MediaPlayerActivity.HISTORICAL_STREAM, stream);
        bundle.putString(MediaPlayerActivity.MEDIA_URL, stream.getFullHistoricalVideoUrl());
        if (stream.getCreatedBy() != null) {
            bundle.putString(MediaPlayerActivity.USER_ID, stream.getCreatedBy().getUuid());
        }
        bundle.putString(MediaPlayerActivity.THUMBNAIL_URL, stream.getFullThumbnailUrl());
        playbackIntent.putExtras(bundle);
    ...
    public static void addLocationToStream(final Context context, final Stream stream, final EventBus eventBus) {
        DeviceLocation.getLastKnownLocation(context, false, new DeviceLocation.LocationResult() {
            ...
            public void gotLocation(Location location) {
                com.mobileman.moments.android.backend.model.Location locationHolder = new com.mobileman.moments.android.backend.model.Location();
                locationHolder.setLatitude(location.getLatitude());
                locationHolder.setLongitude(location.getLongitude());
                stream.setLocation(locationHolder);
...
//    }
     ...
     * @return
     */
    public static KickflipApiClient getApiClient(Context context, FBUser fbUser, KickflipCallback callback) {
        checkNotNull(sClientKey);
        checkNotNull(sClientSecret);
        if (sKickflip == null || !sKickflip.getConfig().getClientId().equals(sClientKey)) {
            checkNotNull(fbUser);
            sKickflip = new KickflipApiClient(context, fbUser, callback);
    ...
    private static void setupDefaultSessionConfig() {
        ...
        Kickflip.setSessionConfig(new SessionConfig.Builder(outputLocation, Constants.DEFAULT_HLS_SEGMENT_DURATION)
