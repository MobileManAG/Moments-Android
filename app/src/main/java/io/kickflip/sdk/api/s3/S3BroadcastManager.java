...
public class S3BroadcastManager implements Runnable {
    ...
    public void queueUpload(final String bucket, final String key, final File file, boolean lastUpload) {
        if (VERBOSE) Log.i(TAG, "Queueing upload " + key + "(" + file.getAbsolutePath() + ")");

        final PutObjectRequest por = new PutObjectRequest(bucket, key, file);
        por.setGeneralProgressListener(new ProgressListener() {
            final String url = key; //"https://" + bucket + ".s3.amazonaws.com/" + key;
            ...
            public void progressChanged(com.amazonaws.event.ProgressEvent progressEvent) {
                try {
                    if (progressEvent.getEventCode() == ProgressEvent.STARTED_EVENT_CODE) {
                        ...
                        boolean isLastSegment = (mQueue.size() == 0);
                        if (VERBOSE)
                            Log.i(TAG, "Uploaded " +  (isLastSegment ? "last " : "") + file.length() / 1000.0 + " KB in " + (uploadDurationMillis) + "ms (" + bytesPerSecond / 1000.0 + " KBps)" + " - "+ file.getAbsolutePath());

                        mBroadcaster.onS3UploadComplete(new S3UploadEvent(file, url, bytesPerSecond));
                    } else if (progressEvent.getEventCode() == ProgressEvent.FAILED_EVENT_CODE) {
                        Log.w(TAG, "Upload failed for " + url + " - " + file.getAbsolutePath());
                    }
        ...
        if (key.contains(Stream.HISTORICAL_VIDEO_FILENAME)) {   // do a hack and prioritize the vod file
            if (mQueue.size() > 0) {
                LinkedBlockingQueue<Pair<PutObjectRequest, Boolean>> tempQueue = new LinkedBlockingQueue<>();
                mQueue.drainTo(tempQueue);
                if (mQueue.size() != 0) {
                    Log.e(TAG, "ERROR! Emptying queue FAILED!!!");
                } else {
                    mQueue.add(new Pair<>(por, false));
                    Pair<PutObjectRequest, Boolean> tempObject = null;
                    while (tempQueue.size() > 0) {
                        tempObject = tempQueue.poll();
                        if (tempObject != null) {
                            if (tempQueue.isEmpty()) {
                                tempObject = new Pair<>(tempObject.first, true);
                                mQueue.add(tempObject);
                            } else {
                                mQueue.add(tempObject);
                            }
                        }
    ...
    private ObjectMetadata generateMetadataForFile(ObjectMetadata objectMetadata, File file) {
        ...
        String md5ChecksumString = null;
        try {
            md5ChecksumString = MD5CheckSum.getMD5Checksum(file.getAbsolutePath());

            Log.v(TAG, "CheckSum: " + md5ChecksumString);
        } catch (Exception lException) {
            lException.printStackTrace();
        }

        if (md5ChecksumString != null) {
            objectMetadata.setContentMD5(md5ChecksumString);
        }
    ...
    public void run() {
        try {
            ...
            while (!lastUploadComplete) {
                Pair<PutObjectRequest, Boolean> requestPair = mQueue.poll(mBroadcaster.getSessionConfig().getHlsSegmentDuration(), TimeUnit.SECONDS);
                if (requestPair == null) {
                    requestPair = mQueue.poll(mBroadcaster.getSessionConfig().getHlsSegmentDuration(), TimeUnit.SECONDS);
                }
                if (requestPair == null) {
                    requestPair = mQueue.poll(mBroadcaster.getSessionConfig().getHlsSegmentDuration(), TimeUnit.SECONDS);
                }
                ...
                if(requestPair != null) {
                    ...
                    objectMetadata = generateMetadataForFile(request.getMetadata(), request.getFile());
                    ...
                    try {
                        ...
                        mBroadcaster.onTryingToReconnect(false);
                        ...
                        Log.e(TAG, "upload error from S3");
                        if (e.getMessage() != null) {
                            Log.e(TAG, e.getMessage());
                        }
                        e.printStackTrace();
                        mBroadcaster.onTryingToReconnect(true);

                    }
                } else if (timeoutExpired) {
                    if (VERBOSE) Log.e(TAG, "Reached end of Queue before processing last segment!");
                    lastUploadComplete = true;
                    mBroadcaster.onS3BroadcastFatalError("Error - missing video recording data");
                }
            }
